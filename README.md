...
3. **気分**: エナジーの代わりに「普通/眠い/ご機嫌/お腹すいた/お金が欲しい」のいずれかの気分を持ち、時間で変化する。眠いと街へ戻って休む。鳥をタップすると今の気分と一言(行きたい場所のヒント)が見られる。
4. **依頼掲示板**: プレイヤーが欲しい素材・数量・報酬(ゴールド)を指定して依頼を出す。手が空いている鳥が、自分の性格・気分・報酬額から判断して受けるかどうかを決める(命令ではなく募集。受けてもらえないこともある)。依頼を受けた鳥は該当する資源を優先して集め、完了すると素材が手に入り報酬ゴールドを支払う。
5. **戦闘・採掘・お宝**: 敵との戦闘・資源の採掘・お宝の開封は鳥が現地に到着すると自動で行われる(タップ操作は不要)。倒れた鳥はその場でしばらく休むと自然回復する(消滅・全滅はない)。
6. **永続化**: 所持ゴールドと素材(木材/鉱石/キノコ)は端末に保存(AsyncStorage)。街・鳥の状態(位置・気分・依頼)は現状セッション内のみ(今後の課題)。
...
- 敵・採掘オブジェクトのリスポーン（現状は倒す/採り切ると復活しない）
